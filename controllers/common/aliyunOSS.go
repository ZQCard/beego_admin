...
type AliyunOSSController struct {
	beego.Controller
}
// 请填写您的AccessKeyId。
var accessKeyId string = beego.AppConfig.String("aliyun_oss_access_key")
// 请填写您的AccessKeySecret。
var accessKeySecret string = beego.AppConfig.String("aliyun_oss_access_secret")
// host的格式为 bucketname.endpoint ，请替换为您的真实信息。
var host string = "http://" + beego.AppConfig.String("aliyun_oss_bucket") + "." + beego.AppConfig.String("end_point")
// callbackUrl为 上传回调服务器的URL，请将下面的IP和Port配置为您自己的真实信息。
var callbackUrl string = beego.AppConfig.String("aliyun_oss_callBack")
// 用户上传文件时指定的前缀。
...
func getMD5FromNewAuthString(r *http.Request) ([]byte, error) {
	...
	if strCallbackBody == ""{
		r.ParseForm()
		strCallbackBody = "filename="+r.Form.Get("filename")
	}
...
// 公共请求不需要CSRF
func (c *AliyunOSSController)Prepare()  {
	c.EnableXSRF = false
}

// 获取policy
func (c *AliyunOSSController)GetPolicy()  {
...
// 阿里云回调
func (c *AliyunOSSController)PostCallback()  {
	r := c.Ctx.Request
	w := c.Ctx.ResponseWriter
	w.Header().Set("Access-Control-Allow-Methods", "*")
	w.Header().Set("Access-Control-Allow-Origin", "*")
